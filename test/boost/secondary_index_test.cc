...
void assert_select_count_and_select_rows_has_size(cql_test_env& e, const sstring& rest_of_query, int64_t expected_count) {
    eventually([&] { 
        auto res = e.execute_cql("SELECT count(*) " + rest_of_query).get0();
        assert_that(res).is_rows().with_rows({
            { long_type->decompose(expected_count) }
        });
        res = e.execute_cql("SELECT * " + rest_of_query).get0();
        assert_that(res).is_rows().with_size(expected_count);
    });
}

static constexpr int page_scenarios_row_count = 2 * cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE + 120;
static constexpr int page_scenarios_initial_count = 3;
static constexpr int page_scenarios_window_size = 4;
static constexpr int page_scenarios_just_before_first_page = cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE - page_scenarios_window_size;
static constexpr int page_scenarios_just_after_first_page = cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE + page_scenarios_window_size;    
static constexpr int page_scenarios_just_before_second_page = 2 * cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE - page_scenarios_window_size;
static constexpr int page_scenarios_just_after_second_page = 2 * cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE + page_scenarios_window_size;    

static_assert(page_scenarios_initial_count < page_scenarios_row_count);
static_assert(page_scenarios_window_size < cql3::statements::select_statement::DEFAULT_COUNT_PAGE_SIZE);
...
// 2. In a window from just before and just after `insert`s where executed
//    DEFAULT_COUNT_PAGE_SIZE times
// 3. In a window from just before and just after `insert`s where executed
//    2 * DEFAULT_COUNT_PAGE_SIZE times
// 4. After all `insert`s
void test_with_different_page_scenarios(
    noncopyable_function<void (int64_t)> insert, noncopyable_function<void (int64_t)> validate) {
...
SEASTAR_TEST_CASE(test_secondary_index_on_ck_first_column_and_aggregation) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk FROM t2 WHERE ck = 1 GROUP BY pk").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT pk, ck FROM t2 WHERE ck = 1 GROUP BY pk, ck").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT sum(pk) FROM t2 WHERE ck = 1").get0();
            assert_that(res).is_rows().with_rows({
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk FROM t3 WHERE ck1 = 1 GROUP BY pk").get0();
            assert_that(res).is_rows().with_size(1);
            res = e.execute_cql("SELECT pk, ck1 FROM t3 WHERE ck1 = 1 GROUP BY pk, ck1").get0();
            assert_that(res).is_rows().with_size(1);
            res = e.execute_cql("SELECT pk, ck1, ck2 FROM t3 WHERE ck1 = 1 GROUP BY pk, ck1, ck2").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT avg(ck2) FROM t3 WHERE ck1 = 1").get0();
            assert_that(res).is_rows().with_rows({
...
SEASTAR_TEST_CASE(test_secondary_index_on_pk_column_and_aggregation) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk1, pk2 FROM t1 WHERE pk2 = 1 GROUP BY pk1, pk2").get0();
            assert_that(res).is_rows().with_size(1);
            res = e.execute_cql("SELECT pk1, pk2, ck FROM t1 WHERE pk2 = 1 GROUP BY pk1, pk2, ck").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT min(pk1) FROM t1 WHERE pk2 = 1").get0();
            assert_that(res).is_rows().with_rows({
                { int32_type->decompose(1) }
            });
          });
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk1, pk2 FROM t2 WHERE pk2 = 1 GROUP BY pk1, pk2").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT max(pk1) FROM t2 WHERE pk2 = 1").get0();
            assert_that(res).is_rows().with_rows({
...
SEASTAR_TEST_CASE(test_secondary_index_on_non_pk_ck_column_and_aggregation) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk FROM t WHERE v = 1 GROUP BY pk").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT sum(v) FROM t WHERE v = 1").get0();
            assert_that(res).is_rows().with_rows({
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk FROM t2 WHERE v = 1 GROUP BY pk").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT sum(pk) FROM t2 WHERE v = 1").get0();
            assert_that(res).is_rows().with_rows({
                { int32_type->decompose(int32_t(rows_inserted * (rows_inserted - 1) / 2)) }
            });
          });
        ...
        test_with_different_page_scenarios([&](int64_t current_row) {
          ...
          eventually([&] { 
            auto res = e.execute_cql("SELECT pk FROM t3 WHERE v = 1 GROUP BY pk").get0();
            assert_that(res).is_rows().with_size(1);
            res = e.execute_cql("SELECT pk, ck FROM t3 WHERE v = 1 GROUP BY pk, ck").get0();
            assert_that(res).is_rows().with_size(rows_inserted);
            res = e.execute_cql("SELECT max(ck) FROM t3 WHERE v = 1").get0();
            assert_that(res).is_rows().with_rows({
                { int32_type->decompose(int32_t(rows_inserted - 1)) }
            });
          });
